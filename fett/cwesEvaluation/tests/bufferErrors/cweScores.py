...
def scoreByCWE(SCORES, rows):
    ...
    for test in sorted(results.keys()):
        v = score(SCORES, results[test])
        tab.append([f"{test}", v[0], v[1]])
    return tab

def scoreAllTests(SCORES, customScorer, logs, testsDir):
    tab = count.tabulate(SCORES, customScorer, testsDir, count.QEMU_FPGA_LOOKFOR)
    return scoreByCWE(SCORES, tab)
