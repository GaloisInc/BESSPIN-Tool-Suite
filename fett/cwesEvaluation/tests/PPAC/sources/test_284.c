...
//---------------- FreeRTOS test ------------------------------------------------------
    ...
    #define NUM_OF_TEST_PARTS 4 //test_284 has 4 parts
    ...
    static void vTask284 (void *pvParameters);
    void main_test_284 (void);
    ...
    void main (void) {
        ...
        funcReturn = xTaskCreate(vTask284, "main:task284", configMINIMAL_STACK_SIZE * STACKSIZEMUL, NULL, xMainPriority, NULL);
        if (funcReturn != pdPASS) {
            onPrintf ("<INVALID> Failed to create <main:task284>. [ret=%d].\n",funcReturn);
            return;
        } else {
            onPrintf (">>> [main:] Created <main:task284>.\n");
        }
    ...
    static void vTask284 (void *pvParameters) {
        ...
        subjectCN[xBytesReceived] = '\0'; //add the null termination
            ...
            if (usersList == NULL) {
                ...
                onPrintf ("<BREACHED-LIST> [target-server]: Got the structure pointer!\n");
                //add the username to the front of the list
                strcpy (usersList[0], subjectCN);
            }
        #endif
        ...
        if (getServicePermission(subjectCN) == uTRUE) {
            ...
            if (logQueue == NULL) {
                ...
                onPrintf ("<BREACHED-LOG> [target-server]: Got the queue handle!\n");
                logQueue = NULL; //that's bad
            }
        #endif
