#! /usr/bin/env bash
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# Runs testgen for numericErrors
...
if [ $pocExploitsMode -ne 1 ]; then #normal mode-- testgen
    mkdir $testsDir
    touch $testsDir/testsParameters.h
    cp $vBASE_DIR/sources/test_*.c $testsDir/
    cp $vBASE_DIR/sources/Makefile.test_specific $testsDir/
...
if [ $pocExploitsMode -ne 1 ] && [[ $partialRun != "onlyBuild" ]]; then
    ...
    $gSCRIPTS/scoreTests.py $vSCRIPTS/cweScores.py $testsDir $reportFile $useCustomScoring $customizedScoringConfigFile
