...
def setEthAdaptorName ():
    def tryToGetEthAdaptorName(ipOrMac,address=None):
        if (ipOrMac == 'MAC'):
            family = psutil.AF_LINK
        elif (ipOrMac == 'IP'):
            family = socket.AF_INET
        else:
            logAndExit(f"<fpga.tryToGetEthAdaptorName> called with unrecognized argument <{ipOrMac}>.",
    ...
    if (not macAddressMatch):
        ...
        if (tryToGetEthAdaptorName('MAC', address=macAddress)):
            return
        else:
    ...
    if (tryToGetEthAdaptorName('IP', address=getSetting('fpgaIpHost'))):
        return
    #Failed. Try again
