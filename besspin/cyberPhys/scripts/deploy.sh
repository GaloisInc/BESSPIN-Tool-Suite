...
IP=10.88.88.3
echo "Deploying to ${IP}"
rsync --progress -a --exclude=.git/* --exclude=FreeRTOS --exclude=BESSPIN-Environment --exclude=BESSPIN-Voter-Registration --exclude=workDir BESSPIN-Tool-Suite galoiuser@${IP}:~/
ssh galoiuser@${IP} "cd /home/galoiuser/BESSPIN-Tool-Suite/besspin/cyberPhys/cyberphyslib && sudo python3 setup.py install"
