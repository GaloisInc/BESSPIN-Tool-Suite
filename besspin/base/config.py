...
def genProdConfig(configFileSerialized, configFile):
    ...
    prodTemplatePath = os.path.join(getSetting('repoDir'),'besspin','base','utils','productionTemplate.json')
    ...
    for vulClass in getSetting('vulClasses'): #load settings per vulClass
        ...
        if ((vulClass not in ['bufferErrors']) 
            ...
            warnAndLog(f"{vulClass}: <runAllTests> is enabled; The <test_*> settings in <{configCWEsPath}>"
                f"will be ignored.") 
            # The ignoring happens in besspin/cwesEvaluation/common.py:isTestEnabled() which checks
        ...
        setupEnvData = safeLoadJsonFile(os.path.join(getSetting('repoDir'),'besspin','cwesEvaluation',vulClass,'setupEnv.json'))
