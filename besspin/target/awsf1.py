...
from besspin.target.fpga import fpgaTarget
...
def installKernelModules():
    ...
    if (isEqSetting('pvAWS','firesim')):
        ...
        _sendKmsg (f"BESSPIN-firesim: Installing nbd.ko.")
        sudoShellCommand(['insmod', f"{awsModPath}/xdma.ko", 'poll_mode=1'])
        _sendKmsg (f"BESSPIN-firesim: Installing xdma.ko.")

    elif (isEqSetting('pvAWS', 'connectal')):
        sudoShellCommand(['insmod', f"{awsModPath}/pcieportal.ko"])
        _sendKmsg (f"BESSPIN-connectal: Installing pcieportal.ko.")
        sudoShellCommand(['insmod', f"{awsModPath}/portalmem.ko"])
        _sendKmsg (f"BESSPIN-connectal: Installing portalmem.ko.")
...
def prepareFiresim():
    """prepare the firesim binaries for the BESSPIN work directory"""
    copyAWSSources()
...
def startRemoteLogging (target):
    ...
    if (isEqSetting('osImage','debian')):
        if (not target.restartMode):
            ...
            # Create conf file
            syslogConfName = "logBesspin.conf"
        ...
        if (not target.restartMode):
            ...
            target.runCommand(f'echo "*.*     @{target.ipHost}:{getSetting("rsyslogPort")}" > /etc/syslog.d/logBesspin.conf')
        ...
        if (webserver in target.appModules):
            ...
            if (not target.restartMode):
                target.runCommand(f'printf "{remoteLogsCommands}" > {nginxSrc}/nginx/conf/sites/remoteLogBesspin.conf',erroneousContents=["Unmatched", "No such file or directory"])
