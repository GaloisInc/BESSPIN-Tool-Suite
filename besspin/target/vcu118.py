...
from besspin.target.common import *
from besspin.target.fpga import fpgaTarget, failStage
from besspin.target.build import getTargetIp, getTargetMac, freeRTOSBuildChecks, buildFreeRTOS
...
class vcu118Target (fpgaTarget, commonTarget):
    ...
    def interact (self):
        ...
        if (self.osImage=='FreeRTOS'):
            ...
            printAndLog ("Please use \"--ctrlc\" for interrupts. (Ctrl-C would exit the whole BESSPIN tool).")
        if (self.userCreated):
    ...
    def getOpenocdCustomCfg(self,isReload=False):
        ...
        if (isEnabled('IsThereMoreThanOneVcu118Target')):
            ...
            # In case of a single board, the openocd configuration in `besspin/target/utils/openocd_vcu118.cfg`
    ...
    def buildSmokeElfForUartSearch(self,hwId):
        ...
        #Copy the C files
        copyDir(os.path.join(getSetting('repoDir'),'besspin','target','utils','srcMinimalFreeRTOS'),buildDir,copyContents=True)

        #The needed configs
        fConfig = ftOpenFile(os.path.join(buildDir,'besspinFreeRTOSConfig.h'),'a')
        fConfig.write(f"#define\tBESSPIN_HWID\t\"{hwId}\"\n")
        fConfig.close()

        configHfile = ftOpenFile (os.path.join(buildDir,'besspinUserConfig.h'),'a')
...
def prepareOsBinaryForFlash(targetId=None):
    ...
    copyDir(os.path.join(getSetting('repoDir'),'besspin','target','utils','vcu118FlashBuild'),buildDir,copyContents=True)
