...
def buildApps ():
    ...
    if (isEnabled('buildApps') and isEnabled('isUnix')):
        targetUtilsDir = os.path.join(getSetting('repoDir'),'besspin','target','utils')
...
def prepareFreeRTOS():
    ...
    if (not isEnabled('buildApps')): #just fetch the image
        besspin.target.build.importImage()
    else: #build it
        ...
        for xSetting,xMacro in listConfigParams:
            try:
                ...
                logAndExit(f"Invalid type in populating <besspinUserConfig.h>.",exc=exc,exitCode=EXIT.Dev_Bug)
        ...
        configHfile.write(f"#define BESSPIN_MODE \'{getSetting('mode')[0].upper()}\'\n")
        configHfile.close()

        besspin.target.build.prepareFreeRTOSNetworkParameters()
        besspin.target.build.buildFreeRTOS()
    return
