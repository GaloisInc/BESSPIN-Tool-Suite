...
/*
 * For the DARPA BESSPIN Programme, the TFTP Server has been modidied as follows:
 *
    ...
    *file_size = 0;
    ...
    if (xTFTPRxSocket != FREERTOS_INVALID_SOCKET)
    {
        ...
        do
        {
            ...
            {
                /* Timed out. */
                besspinPrintf("(Error)~  TFTP Timeout.\n");
                xRetries++;

                if (xRetries > ipconfigTFTP_MAX_RETRIES)
                {
                    besspinPrintf("(Error)~  TFTP Retry limit exceeded.\n");
                    xReturn = pdFAIL;
                }
            }
            else if (lBytes < 0) // some other error occurred
            {
                besspinPrintf("(Error)~  UDP receive error code %d", lBytes);
                xReturn = pdFAIL;
            }
            else
            {
                    ...
                    (pxClient->sin_port == xClient.sin_port))
                {
                    ...
                    {
                        besspinPrintf("(Info)~  TFTP file too large for receiving "
                                   "buffer\n");
                ...
                else
                {
                    ...
                    besspinPrintf("(Info)~  TFTP packet addresses or sequence numbers do not match\n");
                    besspinPrintf("(Info)~  TFTP packet opcode %d, BNs (%d, %d), IPs (%s, %s), ports (%d, %d)\n",
        ...
        besspinPrintf("(Info)~  TFTP Closing connection.\n");
    ...
    else
    {
        ...
        // caller responsible
        besspinPrintf("(Error)~  TFTP Could not create socket to receive file.\n");
    }
                                           ...
                                           int32_t lBytes)
{
    ...
    {
        besspinPrintf("(Error)~  TFTP write request header too short\n");
    ...
    if (Terminators_OK)
    {
        ...
        if (Mode_OK)
        {
            ...
            besspinPrintf("(Info)~  validateWriteRequest pcFileName is %s\n",
                       pcFileName);
            besspinPrintf("(Info)~  validateWriteRequest fnlen is %d\n",
                       (int)fnlen);
        ...
        {
            besspinPrintf("(Error)~  TFTP transfer mode is not octet\n");
            prvSendTFTPError(xSocket, pxClient, eIllegalTFTPOperation);
            // caller responsible
            pcFileName = NULL;
        }
    ...
    {
        besspinPrintf("(Error)~  TFTP mal-formed write request header\n");
        prvSendTFTPError(xSocket, pxClient, eIllegalTFTPOperation);
        // caller responsible
        pcFileName = NULL;
    }
                               ...
                               uint32_t file_name_len) // in
{
    ...
    {
        besspinPrintf("(Error)~  TFTP failed to create Listening Socket\n");
        // caller responsible
        return 0;
    }
    ...
    if (Bind_Result != 0)
    {
        besspinPrintf("(Error)~  TFTP socket bind failed with return code %d\n",
                   (int)Bind_Result);
        // caller responsible
        return 0;
    }
    ...
    /* Look for the start of a new transfer on the TFTP port.  ulFlags has
       ...
       needed. */
    besspinPrintf("(Info)~  TFTP waiting for initial Request Message\n");
    ...
    if (lBytes >= 0)
    {
        if (lBytes >= TFTP_Write_Request_Min_Length)
        {
            ...
            besspinPrintf("(Info)~  TFTP received a request. First header bytes "
                       "are %2x %2x\n",
            ...
            else
            {
                ...
                besspinPrintf("(Error)~  TFTP not a Write Request\n");
        ...
        {
            /* message too short. */
            besspinPrintf("(Error)~  TFTP initial request message too short\n");
            prvSendTFTPError(xTFTPListeningSocket, &xClient,
                             eUnknownTransferID);
        }
    ...
    else
    {
        ...
        if (lBytes != FREERTOS_EWOULDBLOCK)
        {
            ...
            besspinPrintf("(Warning)~  TFTP recvfrom() failed with return code %d\n",
                       (int)lBytes);
        }
    }
    ...
    {
        besspinPrintf("(Info)~  TFTP File Receive failed. Sanitizing buffers\n");
