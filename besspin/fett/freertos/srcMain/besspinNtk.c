...
void startNetwork () {
    ...
    if (funcReturn != pdPASS) {
        besspinPrintf ("(Error)~  startNetwork: Failed to initialize network. [ret=%d].\n",funcReturn);
        exitBesspin(1);
    } else {
        besspinPrintf ("(Info)~  startNetwork: Network IP initialized successfully!.\n");
    }
}
...
void vApplicationIPNetworkEventHook(eIPCallbackEvent_t eNetworkEvent) {
    besspinPrintf ("(Info)~  NtkHook: A network event has been detected! (eventNum=%d).\r\n",eNetworkEvent);

    /* If the network has just come up...*/
    if ((eNetworkEvent == eNetworkUp) && (isNetworkUp == uFALSE)) //Network never was up
    {
        besspinPrintf ("(Info)~  NtkHook: Network is up!\r\n");
        char cBuffer[16];
        ...
        besspinPrintf("\t\tIP Address: %s\r\n", cBuffer);

        FreeRTOS_inet_ntoa(ulNetMask, cBuffer);
        besspinPrintf("\t\tSubnet Mask: %s\r\n", cBuffer);

        FreeRTOS_inet_ntoa(ulGatewayAddress, cBuffer);
        besspinPrintf("\t\tGateway Address: %s\r\n", cBuffer);

        FreeRTOS_inet_ntoa(ulDNSServerAddress, cBuffer);
        besspinPrintf("\t\tDNS Server Address: %s\r\n\r\n", cBuffer);

        // notify main
        if (xMainTask == NULL) {
            besspinPrintf ("(Error)~  NtkHook: Unable to get the handle of <main:task>.\n");
            exitBesspin (1);
        }
        BaseType_t funcReturn = xTaskNotify( xMainTask, NOTIFY_SUCCESS_NTK ,eSetBits);
        if (funcReturn != pdPASS) {
            besspinPrintf ("(Error)~  NtkHook: Failed to notify <main:task>!\n");
            exitBesspin (1);
        }
    ...
    if (eNetworkEvent == eNetworkDown) {
        besspinPrintf ("(Error)~ NtkHook: Network is Down!\n");
        exitBesspin (1);
    }
