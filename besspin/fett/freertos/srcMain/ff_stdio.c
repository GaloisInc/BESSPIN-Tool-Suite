...
int ff_init( void )
{
    ...
    if (ff_mutex == NULL)
      {
        besspinPrintf ("(Error)~ failed to allocate memory for ff_mutex\r\n");
        exitBesspin (1);
    ...
    (void) xSemaphoreGive (ff_mutex);
        ...
        besspinPrintf ("(Info)~ ff_init: mkfs returns %d\n", (int) res);
        res = f_mount(&FatFs, "", 0); 
        besspinPrintf ("(Info)~ ff_init: mount returns %d\n", (int) res);
        return res;
        ...
        if (IceblkDevInstance.disk_present) {
            besspinPrintf ("(Info)~ ff_init: Disk present. Mounting filesystem...\r\n");
            return f_mount(&FatFs, "", 0);
        } else {
            besspinPrintf ("(Error)~ iceclk disk is not present\r\n");
            return 1;
        }
...
FF_FILE *ff_fopen( const char *pcFile, const char *pcMode )
{
    ...
    if (strlen(pcFile) > ffconfigMAX_FILENAME) {
        besspinPrintf("(Error)~  ff_open: Length of filename should be <= %d.\r\n",ffconfigMAX_FILENAME);
        exitBesspin (1);
    ...
    if (file == NULL) {
        besspinPrintf("(Error)~  ff_open: Failed to malloc <FF_FILE>.\r\n");
        exitBesspin (1);
        // caller responsible
        return NULL;
    }
        ...
        if ((strcmp(pcMode,"r") == 0) || (strcmp(pcMode,"rb") == 0)) {
            ...
            besspinPrintf ("(Error)~  ff_fopen: pcMode:<%s> is not implemented.\r\n",pcMode);
            return NULL;
        }
        int res = f_open(&(file->fatfsFile), pcFile, mode);
        if (res != FR_OK) {
            besspinPrintf ("ff_fopen: Failed to open <%s>. [ret=%d]\r\n",pcFile,res);
            return NULL;
        }
        ...
        if (!sdlib_open(pcFile,pcMode)) {
            besspinPrintf ("ff_fopen: Failed to open <%s>.\r\n",pcFile);
            return NULL;
        }
...
size_t ff_fread( void *pvBuffer, size_t xSize, size_t xItems, FF_FILE * pxStream )
{
    ...
    bytes = xSize * xItems;
        ...
        if (res != FR_OK) {
            besspinPrintf ("(Error)~ ff_fread: failed to read from file. [ret =%d]\r\n",res);
...
size_t ff_fwrite( void *pvBuffer, size_t xSize, size_t xItems, FF_FILE * pxStream )
{
    ...
    bytes = xSize * xItems;
        ...
        if (res != FR_OK) {
            besspinPrintf ("(Error)~ ff_fwrite: failed to write to file. [ret=%d]\r\n",res);
