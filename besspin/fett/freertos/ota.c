...
void Write_Payload_To_FS(size_t fsize)
{
    ...
    if (fd != NULL)
    {
        ...
        {
            // T1D3
            besspinPrintf("(Error)~  vOta: file write failed. [written=%ld, "
                       "fsize=%ld].\r\n",
                       written, fsize);
        ...
        {
            // T1D3
            besspinPrintf("(Error)~  vOta: file close failed\n");
        }
    }
    else
    {
        ...
        // T1D3
        besspinPrintf("(Error)~  vOta: failed to open %s\r\n", OTA_FILENAME);
    }
...
{
    size_t n = fsize;
    besspinPrintf("(Info)~  vOta: Received payload (up to first 256 bytes) is\n");
    ...
    {
        besspinPrintf("%c ", file_buffer[ED25519_SIG_SIZE + i]);
        if ((i % 16) == 15)
        {
            besspinPrintf("\n");
        }
...
void Receive_And_Process_One_OTA_Request(ed25519_key *pk)
{
    ...
    {
        besspinPrintf("(Info)~ OTA received a file of %d bytes\n",
                   (int)received_file_size);
        besspinPrintf("(Info)~ OTA requested file name is %s\n",
                   tftp_filename);
        besspinPrintf(
        ...
        if ((r == 0) && (signature_ok == 1))
        {
            ...
            {
                besspinPrintf("(Info)~  vOta: Signed STOP message received\n");
                setStopRequested();
            }
            else
            {
                besspinPrintf("(Info)~  vOta: Signature is OK\n");
        ...
        else
        {
            ...
            besspinPrintf("(Info)~  vOta: Signature is NOT OK\n");
        }
    }
    else
    {
...
void Ota_Worker(ed25519_key *pk)
{
    ...
    besspinPrintf("(Info)~  vOta: Ota_Worker returns after STOP message\n");
}
...
void vOta(void *pvParameters)
{
    ...
    besspinPrintf("(Info)~  vOta: Starting OTA...\r\n");

    besspinPrintf("(Info)~  vOta: task initial SHWM is %u\n",
    ...
    besspinPrintf("<TFTP-SERVER-READY>\n");
    ...
    besspinPrintf("(Info)~  vOta: Exiting OTA...\r\n");
    besspinPrintf("(Info)~  vOta: task final SHWM is %u\n",
               (uint32_t) uxTaskGetStackHighWaterMark(NULL));
