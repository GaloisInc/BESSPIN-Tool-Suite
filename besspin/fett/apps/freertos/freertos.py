...
def rtosRunCommand (target,command,endsWith=[],expectedContents=None,erroneousContents=[],exitOnError=True,timeout=60,suppressErrors=False,endOfApp=False,tee=True):
    ...
    retCommand = target.runCommand(command,endsWith=[">>>End of Besspin<<<"] + endsWith,
        expectedContents=expectedContents,erroneousContents=erroneousContents + ['(Error)','EXIT: exiting BESSPIN with code <1>'],exitOnError=exitOnError,
        timeout=timeout,suppressErrors=suppressErrors,tee=teeFile)

    if ((retCommand[3] == 0) and (not endOfApp)): #BESSPIN exited prematurely
...
def prepareAssets ():
    ...
    for xAsset in listAssets:
        ...
        assetsHfile.write(besspin_xxd_i(os.path.join(assetsPath,xAsset)))
...
@decorate.debugWrap
@decorate.timeWrap
def besspin_xxd_i (binPath,check83=True):
    """
    ...
    try:
        ...
        logAndExit (f"besspin_xxd_i: Failed to generate a header from <{binPath}>.",exc=exc,exitCode=EXIT.Files_and_paths)
