...
from besspin.target.build import prepareFreeRTOSNetworkParameters
import subprocess, os, re, time, random

@decorate.debugWrap
def prepareFreeRTOSforPPAC (fHeader):
    # copy extra sources + main FreeRTOS program over
    ppacDir = os.path.join(getSetting('repoDir'),'besspin','cwesEvaluation','PPAC')
    ...
    # Needs besspinFreeRTOSIPConfig.h
    cp (os.path.join(sourcesDir,'libFreeRTOS','besspinFreeRTOSIPConfig.h'),
