...
def test_307 (target,binTest):
    ...
    if (target.osImage == 'debian'):
        ...
        if (target.testPart == 1):
            ...
            for iSubPart, nOfSubPart in enumerate([1, nAttempts, nAttempts+1]):
                ...
                if (retAuthAttempts[1] == 'INVALID'):
                    outLog += target.runCommand("getTargetDump",endsWith=">>>End of Besspin<<<",timeout=10,exitOnError=False)[1]
        ...
        if (">>>End of Besspin<<<" not in outLog):
            retFinish = target.runCommand("allProgram",endsWith=">>>End of Besspin<<<",exitOnError=False,timeout=20)
            outLog += retFinish[1]
