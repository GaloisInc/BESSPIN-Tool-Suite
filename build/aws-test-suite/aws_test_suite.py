"""
AWS Instance Management Functions
"""
import logging
import boto3
...
def get_ami_id_from_name(ami_name):
    ...
    assert len(response['Images']) == 1, f"No unique images found '{response['Images']}'"
...
def launch_instance(ami_name,
    ...
    security_group = client.describe_security_groups(
        Filters=[{'Name':'group-name', 'Values': [security_group_name]}])['SecurityGroups'][0]['GroupId']

    image_id = get_ami_id_from_name(ami_name)

    # get subnets and choose a public one
    subnets = list(vpc[0].subnets.filter(Filters=[{'Name': 'tag:Name', 'Values': ['*Public*']}]))
    ...
    try:
        ...
        if tags:
            optional_kwargs['TagSpecifications'] = ([] if tags is None else [
                            {
