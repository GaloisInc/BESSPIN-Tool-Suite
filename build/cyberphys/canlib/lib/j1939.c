...
uint8_t test_pgn(uint32_t pgn){
  if (pgn > 0x03FFFF) return INVALID_PGN;
  return SUCCESS;
}
...
uint8_t send_can_message(cyberphys_socket_t socket, cyberphys_sockaddr_t servaddr, uint32_t pgn,
                         void *message, size_t message_len) {
  /* TODO: doesn't work for FreeRTOS */
  if (test_pgn(pgn))
    return INVALID_PGN;
  if (message_len > 1785)
    return INVALID_BUFF_SIZE;

  if (message_len <= 8) { /* message fits in a single packet */
    can_frame frame;
...
uint8_t recv_can_message(cyberphys_socket_t socket, void *rmessage, size_t *rmessage_len) {
  uint32_t nbytes;
  can_frame *frame;
  ...
  if (nbytes ==
      sizeof(can_frame)) { /* extract data field from single can frame */
    *rmessage_len = frame->can_dlc;
    memcpy(rmessage, (void *)&frame->data[0], *rmessage_len);
    return SUCCESS;
  } else { /* perform multiple can frame protocol */
